...
void Player::fight(Enemy& enemyPractice, Maze& maze, Parser parser, std::string& BG){
	...
	while (intHealth > 0 && enemyPractice.getHealth() > 0){
		...
		if (enemyAttack == true){
			...
			if (intHealth <= 0){
				...
				//ironic that the enemy dies either way, but whatever makes them disappear
				enemyPractice.die();
				break;
			}
			else
...
void Player::chooseFunctionFromMap(int int_room_type, Maze& maze, Enemy& enemy, std::string& BG){
	int randNum = rand() % 3;
...
void Player::playDay(int in, std::string& BG){
